...
async function main() {
  ...
  const requestedAt = readRequestedAt(
    ...
    "worker-weather-refresh-schedule",
  );
  ...
  if (fieldId && !workspaceId) {
    throw new Error(
      "[worker-weather-refresh-schedule] --field-id requires --workspace-id",
    );
  }
  ...
  const targets = await resolveWorkspaceDispatchTargets({
    runtime,
    workspaceId,
    label: "worker-weather-refresh-schedule",
  });
...
void main().catch((error: unknown) => {
  const message =
    error instanceof Error
      ? error.message
      : "Unknown weather refresh schedule failure";
  console.error(`[worker-weather-refresh-schedule] ${message}`);
  process.exitCode = 1;
});
