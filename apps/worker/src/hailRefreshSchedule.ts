...
async function main() {
  ...
  if (fieldId && !workspaceId) {
    throw new Error(
      "[worker-hail-refresh-schedule] --field-id requires --workspace-id",
    );
  }
  ...
  const targets = await resolveWorkspaceDispatchTargets({
    runtime,
    workspaceId,
    label: "worker-hail-refresh-schedule",
  });
...
void main().catch((error: unknown) => {
  const message =
    error instanceof Error ? error.message : "Unknown hail refresh schedule failure";
  console.error(`[worker-hail-refresh-schedule] ${message}`);
  process.exitCode = 1;
});
