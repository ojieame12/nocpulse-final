...
test("resolveRampAccent: light mode returns rgb() accent, rgba() tint, rgba() deep", () => {
  ...
  assert.ok(
    result.deep.startsWith("rgba(") || result.deep.startsWith("rgb("),
    `deep format unexpected: ${result.deep}`,
  );
});
...
test("resolveSeverityAccent: dark mode uses dark severity palette", () => {
  const modeAccent = "rgb(100,150,200)";
  assert.equal(resolveSeverityAccent(modeAccent, "danger", true), SEV_COLORS_DARK.danger.text);
});
...
test("resolveVitalSeverity: sev=true + icon='up' -> 'warning'", () => {
