...
/**
 * buildFieldDetailModeData — behavioral slice tests
 *
 * These tests verify the behavioral CONTRACT of buildFieldDetailModeData,
 * not the exact strings it produces. They assert on:
 *
 *   - Output shape (hero, vitals, spark, spatialColumns always present)
 *   - Severity derivation logic (which inputs produce which sev)
 *   - Mode differentiation (different modes produce different headlines/vitals)
 *   - Null-surface fallback path (the "unavailable" branch)
 *   - Hover override path (hoveredCell replaces headline, vitals, sub, interpretation)
 *   - Spark length invariant (always 6 from surface, or chart-length from trend)
 *   - spatialColumns invariant (always exactly 3)
 *
 * We do NOT deep-equal the entire output. Label text, interpretation copy,
 * and risk copy are allowed to change without breaking these tests.
 *
 * Import path resolves after Phase 1 extraction creates modeDataBuilder.ts.
 */
import { buildFieldDetailModeData } from "../modeDataBuilder";
import type { ModeKey } from "../FieldDetailPanel";
...
/**
 * Minimal surface with N cells at a given metric value.
 * Enough to activate the "availableValue" branch (surface != null).
 */
...
test("shape: hero has v (number), d (string), u (string), sev (SeverityKey)", () => {
  ...
  assert.ok(
    ["positive", "warning", "danger"].includes(result.hero.sev),
    `hero.sev should be positive|warning|danger, got: ${result.hero.sev}`,
  );
});
...
test("shape: hero.v is clamped between 0 and 1", () => {
  const result = buildFieldDetailModeData(createMinimalInput());
  assert.ok(result.hero.v >= 0, `hero.v should be >= 0, got: ${result.hero.v}`);
  assert.ok(result.hero.v <= 1, `hero.v should be <= 1, got: ${result.hero.v}`);
});

// ── Null-surface fallback ──────────────────────────────────────────
...
test("null surface: headline ends with 'unavailable'", () => {
  const result = buildFieldDetailModeData(createMinimalInput({ mapModel: null }));
  assert.ok(
    result.headline.endsWith("unavailable"),
    `expected headline to end with 'unavailable', got: '${result.headline}'`,
  );
});
...
// ── All 5 modes handle null surface without throwing ───────────────
...
test("all modes: null surface produces valid output for every mode", () => {
  ...
  for (const mode of modes) {
    ...
    assert.ok(result.hero, `mode '${mode}' should produce a hero`);
    assert.ok(result.headline.length > 0, `mode '${mode}' should produce a headline`);
    assert.equal(result.hero.d, "—", `mode '${mode}' with null surface should show '—'`);
  }
});
...
test("different modes produce different headlines with a surface present", () => {
  ...
  const moistureResult = buildFieldDetailModeData(
    createMinimalInput({ mode: "moisture", mapModel }),
  );

  // For ndvi, we need to put the surface in the alternate surfaces since
  // the primary agronomicSurface has metricKey "root-zone-moisture-pct"
  const ndviMapModel = {
    agronomicSurface: {
  ...
  const ndviResult = buildFieldDetailModeData(
    createMinimalInput({ mode: "ndvi", mapModel: ndviMapModel }),
  );

  // Headlines should be mode-specific (moisture talks about "moisture", ndvi about "canopy")
...
test("moisture mode vitals include 'Surface' label", () => {
  ...
  assert.ok(labels.includes("Surface"), `moisture vitals should include 'Surface', got: ${labels}`);
});

test("ndvi mode vitals include 'NDVI' label", () => {
  const mapModel = createSurfaceMapModel({ metricKey: "ndvi" });
  const result = buildFieldDetailModeData(
    createMinimalInput({ mode: "ndvi", mapModel }),
  );
  const labels = result.vitals.map((v) => v.label);
  assert.ok(labels.includes("NDVI"), `ndvi vitals should include 'NDVI', got: ${labels}`);
});

test("ndre mode vitals include 'NDRE' label", () => {
  const mapModel = createSurfaceMapModel({ metricKey: "ndre" });
  const result = buildFieldDetailModeData(
    createMinimalInput({ mode: "ndre", mapModel }),
  );
  const labels = result.vitals.map((v) => v.label);
  assert.ok(labels.includes("NDRE"), `ndre vitals should include 'NDRE', got: ${labels}`);
});
...
test("preseason context: ndvi mode changes headline to preseason messaging", () => {
  ...
  assert.ok(
    result.headline.toLowerCase().includes("preseason"),
    `expected preseason headline, got: '${result.headline}'`,
  );
});
...
test("hover: hoveredCell with matching metricKey overrides headline to include 'cell'", () => {
  ...
  const result = buildFieldDetailModeData(
    createMinimalInput({ mode: "moisture", mapModel, hoveredCell }),
  );
  assert.ok(
    result.headline.toLowerCase().includes("cell"),
    `hovered headline should include 'cell', got: '${result.headline}'`,
  );
});
...
test("hover: hoveredCell overrides vitals to show Delta, Percentile, Anomaly", () => {
  ...
  const labels = result.vitals.map((v) => v.label);
  assert.ok(labels.includes("Delta"), `hovered vitals should include 'Delta', got: ${labels}`);
  assert.ok(labels.includes("Percentile"), `hovered vitals should include 'Percentile', got: ${labels}`);
  assert.ok(labels.includes("Anomaly"), `hovered vitals should include 'Anomaly', got: ${labels}`);
});

test("hover: non-matching metricKey does NOT override vitals", () => {
  const mapModel = createSurfaceMapModel();
  const hoveredCell = {
    metricKey: "ndvi",  // doesn't match moisture mode
    metricValuePct: 35,
  ...
  const labels = result.vitals.map((v) => v.label);
  // Should still have moisture vitals, not hover vitals
  assert.ok(!labels.includes("Delta"), `non-matching hover should not show Delta vitals`);
});

// ── Action urgency override ────────────────────────────────────────
...
test("action urgency: non-Routine urgency overrides derived riskLevel", () => {
  ...
  const result = buildFieldDetailModeData(
    createMinimalInput({ mode: "moisture", mapModel, action }),
  );
...
// ── Spark from trend chart vs surface ──────────────────────────────
...
test("spark: with report trend chart, spark length matches chart point count", () => {
  ...
  const result = buildFieldDetailModeData(
    createMinimalInput({ mode: "moisture", mapModel, report }),
  );
  // Trend chart has 4 points, so spark should be [42, 38, 45, 41]
