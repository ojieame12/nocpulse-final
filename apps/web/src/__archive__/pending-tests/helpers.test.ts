...
/**
 * Helper utilities — characterization tests
 *
 * Every assertion below is derived from reading the ACTUAL implementation
 * in FieldDetailPanel.tsx lines 2163–2408.
 *
 * Import path resolves after Phase 1 extraction creates helpers.ts.
 */
import {
  parseNumericValue,
  hasDisplayValue,
...
// ── parseNumericValue ──────────────────────────────────────────────
// Implementation: if (!value) return null; match = value.match(/-?\d+(\.\d+)?/);
// if (!match) return null; parsed = Number(match[0]); isFinite ? parsed : null
...
test("parseNumericValue: returns null for empty string", () => {
  // Empty string is falsy, so !value → true → null
...
// ── hasDisplayValue ────────────────────────────────────────────────
// Implementation: null → false; trim → empty → false;
// "—" → false; "no data" (case-insensitive) → false; else → true
...
test("hasDisplayValue: '0' → true (zero is a valid display value)", () => {
...
// ── percentile ─────────────────────────────────────────────────────
// Implementation: empty → null; clamp fraction [0,1]; linear interpolation
...
test("percentile: [10, 20, 30] at fraction=0.5 → 20 (median)", () => {
  assert.equal(percentile([10, 20, 30], 0.5), 20);
});

test("percentile: [10, 20, 30] at fraction=0.0 → 10 (min)", () => {
  assert.equal(percentile([10, 20, 30], 0.0), 10);
});

test("percentile: [10, 20, 30] at fraction=1.0 → 30 (max)", () => {
  assert.equal(percentile([10, 20, 30], 1.0), 30);
});

test("percentile: interpolates between adjacent values", () => {
  // [10, 20] at fraction=0.5 → rawIndex = 0.5, lower=10, upper=20, weight=0.5
  // result = 10 + (20 - 10) * 0.5 = 15
...
// ── titleCaseLabel ─────────────────────────────────────────────────
// Implementation: if (!value) return "—"; else charAt(0).toUpperCase() + slice(1)
...
// ── buildSparkFromSurface ──────────────────────────────────────────
// Implementation: null or empty cells → [0,0,0,0,0,0]
// else: sort cells by metricValuePct, return percentiles at [0.1, 0.25, 0.4, 0.6, 0.75, 0.9]
...
test("buildSparkFromSurface: empty cells → [0,0,0,0,0,0]", () => {
  assert.deepEqual(
    buildSparkFromSurface({ cells: [], metricAveragePct: 50 } as any),
    [0, 0, 0, 0, 0, 0],
  );
});
...
test("buildSparkFromSurface: result is monotonically non-decreasing (sorted percentiles)", () => {
  const surface = {
...
// ── splitMetricDisplayParts ────────────────────────────────────────
// Implementation: calls formatMetricDisplayValue; if "—" → {display:"—", unit:""};
// if ends with "%" → strip %, unit="%"; else unit=""
...
// ── Finder functions (null-safe lookups) ───────────────────────────
...
test("findReportChart: out of bounds index → null (via ?? null)", () => {
...
test("findLatestReportChartPointValue: all-null points → null", () => {
  const chart = {
    series: [{ points: [{ value: null }, { value: undefined }] }],
  } as any;
...
test("buildSparkFromReportChart: filters out null values, returns finite values", () => {
  const chart = {
    series: [{
      points: [{ value: 10 }, { value: null }, { value: 30 }],
    }],
  } as any;
...
// ── resolveModeTrendChart ──────────────────────────────────────────
// Implementation: chart[0] = vegetation, chart[1] = moisture
// moisture → chart[1], series[0]
// ndvi → chart[0], series[0]
// ndre → chart[0], series[1]
// ndmi → chart[1], series=null
// radarWetness → chart[1], series[2] ?? series[0]
...
test("resolveModeTrendChart: radarWetness prefers series[2], falls back to series[0]", () => {
  ...
  // No series[2], so falls back to series[0]
...
test("findCropProvenanceValue: finds matching key", () => {
  const crop = {
    provenanceRows: [{ key: "Seed Lot", value: "ABC-123" }],
  } as any;
...
test("resolveAvailableModes: null mapModel → empty array", () => {
  const result = resolveAvailableModes(null);
...
test("isPreseasonOpticalContextSurface: matching label → true", () => {
  assert.equal(
    isPreseasonOpticalContextSurface({ sourceLabel: "preseason-optical-context-v1" } as any),
    true,
  );
});
