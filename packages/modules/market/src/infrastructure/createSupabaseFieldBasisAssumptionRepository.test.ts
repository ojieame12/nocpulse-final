...
import { createSupabaseFieldBasisAssumptionRepository } from "./createSupabaseFieldBasisAssumptionRepository";
...
test("latest filters to null crop_symbol when no crop symbol is requested", async () => {
  ...
  const client = {
    from(table: string) {
      assert.equal(table, "field_basis_assumptions");
  ...
  const repository = createSupabaseFieldBasisAssumptionRepository(client as never);
